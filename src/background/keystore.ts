import type { Backend, KeyStorageMode } from "../shared/types.ts";
...
const KEY_FIELDS: Record<Backend, string> = { jev: "jevApiKey", gliner: "fastinoApiKey" };
...
export async function getApiKey(provider: Backend): Promise<string | null> {
...
export async function saveApiKey(provider: Backend, key: string, mode: KeyStorageMode) {
  await deleteApiKey(provider);
  const area = mode === "local" ? chrome.storage.local : chrome.storage.session;
  await area.set({ [KEY_FIELDS[provider]]: key.trim() });
}

export async function deleteApiKey(provider: Backend) {
  const field = KEY_FIELDS[provider];
