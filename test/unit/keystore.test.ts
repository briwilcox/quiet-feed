...
test("session key wins over a local one; empty values count as no key", async () => {
  ...
  assert.equal(await getApiKey(), "session");
  await chrome.storage.session.set({ jevApiKey: "" });
  assert.equal(await getApiKey(), "local");
